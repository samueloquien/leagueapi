...
from models import User, UserUpdate, Book, BookUpdate
...
@book_router.get("/{id}", response_description="Get a single book by id", response_model=Book)
def find_book(id: str, request: Request):
    if (book := request.app.database["books"].find_one({"_id": id})) is not None:
        return book
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Book with ID {id} not found")

@book_router.put("/{id}", response_description="Update a book", response_model=Book)
...
@book_router.delete("/{id}", response_description="Delete a book")
